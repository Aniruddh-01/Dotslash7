...
def crime(request):
    DATA_DIR = os.path.join(BASE_DIR, 'DataSet/crimes_against_women_2001-2014.csv')
    
    print()
    crimes_df = pd.read_csv(DATA_DIR)
    ...
    def plotmap(crime_name):
        DATA_DIR = os.path.join(BASE_DIR, 'DataSet/india_states.geojson')
        ...
        DATA_DIR = os.path.join(BASE_DIR, 'DataSet/result.json')
        with open(DATA_DIR, 'w') as fil:
            json.dump(js, fil)
        DATA_DIR = os.path.join(BASE_DIR, 'DataSet/result.json')
        with open(DATA_DIR,  'r') as fp:
